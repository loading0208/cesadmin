...
def Searchmyscan():
    if request.method == "POST":
        ...
        try:
            ...
            cur.execute(sql)
            myleave = cur.fetchall()
            return render_template("checkmyscan.html",month=month,mysearch=mysearch,myleave=myleave,row=row,summysearch=len(mysearch),employee=noti.Employee(),notification=noti.Notification() )
        except pyodbc.Error as e:
            print(e)
        finally:
            print("Close")
